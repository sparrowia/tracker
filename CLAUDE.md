...
- **RAID archived view:** "Archived (N)" text link below type tabs; flat list sorted by resolved_at desc; type label, priority, owner, resolved date columns; reopen button. Expanded archived/resolved details render description and meeting notes as rich text so public-form attachment URLs remain clickable. The linkifier must HTML-escape `&`, `<`, `>`, `"`, and `'` before interpolating URLs into `href` attributes.
...
- **Comments:** Below description in expanded detail panels; auto-author from logged-in user; Cmd+Enter posting; file attachments via Supabase Storage bucket `comment-attachments` (public bucket). A staged file is sufficient to enable Post and create a comment with an empty body. Upload and delete paths must both use `attachmentPath()` in `comment-thread.tsx`; rebuilding a path from the original display name or changing its sanitization will orphan storage objects. Posting a comment bumps parent item's `updated_at` to trigger unread indicators for other users.
...
- **`action_items` → `vendors` embeds are ambiguous** (same trap): since `owner_vendor_id` was added, `action_items` has **two** FKs to `vendors` (`vendor_id` and `owner_vendor_id`). Any `vendor:vendors(*)` embed on `action_items` **or the `action_item_ages` view** must be hinted: `vendor:vendors!action_items_vendor_id_fkey(*)`. An un-hinted embed returns `PGRST201` and renders an **empty** project board / vendor view — this exact regression shipped twice. **Lesson: adding a second FK to any table breaks every existing PostgREST embed to the referenced table.** After any schema change like that, grep for existing embeds and run the actual app queries against the live API — `tsc`/`build` will NOT catch it.
- **Adding a column to `action_items` requires recreating `action_item_ages`:** the view selects explicit columns (not `SELECT *`), so a new column won't appear until you `CREATE OR REPLACE VIEW ... WITH (security_invoker = on)` and append the column at the end (CREATE OR REPLACE only allows appending). Forgetting this means the column reads `undefined` in the UI even though it exists in the table.
...
- Generated by `scripts/jira-plain-summary.mjs` as `<Area> - <Kind>`, both from ordered first-match regex tables. Rule-based on purpose: the roadmap re-syncs often, and a label whose wording drifts between syncs is worse than one that is occasionally coarse.
- **First match wins, so ordering is the design** — specific signals sit above generic ones (Security and Compliance above the feature areas; Credit hours above Checkout).
- Coverage on the live 545-ticket board: 130 distinct labels, 5 falling back to `Platform -` / `- Change`. Those 5 are genuinely vague ticket titles ("Requesting", "Mobile scalability").
...
- Surfaces: detail-panel select (with "None" to clear) and the filter box's free-text search. **Not** a board column — if it should become one, use `TYPE_SCOPED_COLUMNS` + `RAID_COL_BACKFILL` as documented below for Issue Type.
...
- Attachment upload failures identify the file and Supabase error, block issue creation, and leave the form in place for retry; never silently submit an issue without a selected attachment.
