-- Two-flag meeting toggle: separate project vs vendor meeting inclusion
-- Project-grouped vendor view: RAID entries in vendor accountability

-- 1. Add two-flag meeting columns
ALTER TABLE action_items
  ADD COLUMN include_in_project_meeting boolean NOT NULL DEFAULT false,
  ADD COLUMN include_in_vendor_meeting boolean NOT NULL DEFAULT false;
...
-- Backfill: copy existing include_in_meeting into include_in_project_meeting
UPDATE action_items SET include_in_project_meeting = include_in_meeting;
UPDATE blockers SET include_in_project_meeting = include_in_meeting;
UPDATE raid_entries SET include_in_project_meeting = include_in_meeting;

-- 2. Update project agenda RPC to use include_in_project_meeting
...
AS $$
  WITH combined AS (
    ...
    WHERE b.project_id = p_project_id
      AND b.include_in_project_meeting = true
    ...
    WHERE a.project_id = p_project_id
      AND a.include_in_project_meeting = true
    ...
    WHERE r.project_id = p_project_id
      ...
      AND r.include_in_project_meeting = true
...
-- 3. Update vendor agenda RPC — add RAID entries
...
AS $$
  WITH scored_items AS (
    ...
    SELECT
      ...
      r.title,
      CASE
        WHEN r.status IN ('blocked', 'at_risk') THEN 'critical'::severity_indicator
        WHEN EXTRACT(DAY FROM now() - r.first_flagged_at) > 14 THEN 'high'::severity_indicator
        ELSE 'normal'::severity_indicator
      END,
      COALESCE(r.impact, r.description),
      r.next_steps,
      ...
      r.status::text,
      r.due_date,
      EXTRACT(DAY FROM now() - r.first_flagged_at)::integer,
      0, -- raid_entries has no escalation_count column
      (
        CASE r.priority
          WHEN 'critical' THEN 100
          WHEN 'high' THEN 75
          ...
          WHEN 'low' THEN 25
        END
        + (EXTRACT(DAY FROM now() - r.first_flagged_at) * 2)
        + CASE
            WHEN r.status IN ('blocked', 'at_risk') THEN 30
            ELSE 0
          END
      )::numeric,
      p.full_name,
...
-- 4. Update vendor_accountability view — add RAID entries
