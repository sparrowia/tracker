-- Remove redundant resolved_at IS NULL checks from agenda RPC.
-- Status checks (NOT IN 'complete', 'closed', etc.) are sufficient.
-- The resolved_at check caused items to be invisible even after status was changed back.
...
AS $$
  WITH combined AS (
    ...
    -- blockers: only those toggled for meeting AND not resolved
    SELECT
      'blocker'::text,
      b.id,
      b.title,
    ...
    -- action_items: only those toggled for meeting AND not complete/paused
    SELECT
      'action_item'::text,
      a.id,
      a.title,
    ...
    -- RAID entries: only those toggled for meeting AND not resolved status
    SELECT
