-- Fix generate_project_agenda_from_selected to exclude resolved/completed items
-- Even if include_in_meeting is true, resolved items should not appear in the agenda
...
AS $$
  WITH combined AS (
    ...
    -- action_items: only those toggled for meeting AND not resolved
    SELECT
      'action_item'::text,
      a.id,
      a.title,
    ...
    -- RAID entries: only those toggled for meeting AND not resolved
    SELECT
