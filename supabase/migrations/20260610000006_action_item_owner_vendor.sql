-- Allow a vendor to be the OWNER of an action item (distinct from the existing
-- vendor_id "associated vendor" field). owner_id (person) and owner_vendor_id
-- (vendor) are mutually exclusive, enforced in the app.

ALTER TABLE public.action_items
  ADD COLUMN owner_vendor_id uuid REFERENCES public.vendors(id) ON DELETE SET NULL;
