  ...
  function saveField(id: string, field: string, value: string) {
    ...
    supabase.from("raid_entries").update(dbUpdates).eq("id", id).select("updated_at").single().then(({ data, error }) => {
      ...
      if (data && profileId) {
        ...
        supabase.from("item_reads").upsert({ profile_id: profileId, entity_type: "raid_entry", entity_id: id, read_at: dbUpdatedAt }, { onConflict: "profile_id,entity_type,entity_id" }).then(() => {});
                  ...
                  {isExpanded && (
                    <div className="bg-yellow-50/25 border-b border-gray-200" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="px-5 pt-4 pb-3 text-base font-semibold text-gray-900 bg-yellow-50/25 flex items-start justify-between gap-2">
                        ...
                        <CopyLinkButton url={`${window.location.origin}/projects/${project.slug}?item=${entry.id}`} />
                      </div>
  ...
  return (
    <div className="flex gap-[10px]">
      ...
      <div className="flex flex-col w-[140px] flex-shrink-0">
        ...
        {project.public_issue_form && activeTab === "issue" && (
          <a
            href={`/issues/${project.slug}`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 px-3 text-xs text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-1"
          >
        ...
        <button
          ...
          className={`${project.public_issue_form && activeTab === "issue" ? "mt-1" : "mt-3"} px-3 text-xs text-left transition-colors ${showArchived ? "text-gray-900 font-medium" : "text-gray-400 hover:text-gray-600"}`}
        >
          Archived ({archivedEntries.length})
        </button>
      </div>
