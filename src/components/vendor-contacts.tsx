...
export function VendorContacts({ initialContacts, vendorId, orgId }: {
  ...
  async function deleteContact(id: string) {
    ...
    if (!error) setContacts((prev) => prev.filter((p) => p.id !== id));
  }
                ...
                {isExpanded && canEdit && (
                  <div className="px-3 pb-3 border-t border-gray-100 pt-2 space-y-2" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex justify-end">
                      <button
                        onClick={() => { if (confirm(`Delete ${person.full_name}?`)) deleteContact(person.id); }}
