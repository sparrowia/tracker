...
export default function CommentThread({ raidEntryId, actionItemId, blockerId, orgId, people, itemTitle, itemType, projectName, projectSlug, ownerId }: CommentThreadProps) {
  ...
  async function handlePost() {
    ...
    if (raidEntryId) {
      supabase.from("raid_entries").update({ updated_at: new Date().toISOString() }).eq("id", raidEntryId).select("updated_at").single().then(({ data }) => {
        if (data && profileId) supabase.from("item_reads").upsert({ profile_id: profileId, entity_type: "raid_entry", entity_id: raidEntryId, read_at: data.updated_at }, { onConflict: "profile_id,entity_type,entity_id" }).then(() => {});
      });
    ...
    if (actionItemId) {
      supabase.from("action_items").update({ updated_at: new Date().toISOString() }).eq("id", actionItemId).select("updated_at").single().then(({ data }) => {
        if (data && profileId) supabase.from("item_reads").upsert({ profile_id: profileId, entity_type: "action_item", entity_id: actionItemId, read_at: data.updated_at }, { onConflict: "profile_id,entity_type,entity_id" }).then(() => {});
      });
    ...
    if (blockerId) {
      supabase.from("blockers").update({ updated_at: new Date().toISOString() }).eq("id", blockerId).select("updated_at").single().then(({ data }) => {
        if (data && profileId) supabase.from("item_reads").upsert({ profile_id: profileId, entity_type: "blocker", entity_id: blockerId, read_at: data.updated_at }, { onConflict: "profile_id,entity_type,entity_id" }).then(() => {});
      });
